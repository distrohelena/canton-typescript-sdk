...
function exportedSymbols(filePath) {
    ...
    for (const match of source.matchAll(declarationPattern)) {
        ...
        const symbol = symbols.get(name) ?? { type: false, value: false, service: false, message: false };
        symbol.type ||= ["interface", "type", "class", "enum"].includes(kind);
        symbol.value ||= ["class", "const", "enum", "function"].includes(kind);
        symbol.service ||= kind === "const" && new RegExp(`export\\s+const\\s+${name}\\s*=\\s*new\\s+ServiceType\\s*\\(`).test(source);
        symbol.message ||= kind === "const" && new RegExp(`export\\s+const\\s+${name}\\s*=\\s*new\\s+[A-Za-z_$][\\w$]*\\$Type\\s*\\(`).test(source);
...
function renderNamespace(name, node, indent = "") {
    ...
    for (const module of node.modules.filter(
        ...
        for (const [symbolName, symbol] of module.symbols) {
            ...
            if (symbol.value) {
                const annotation = symbol.service ? ": ServiceType" : symbol.message ? `: MessageType<${module.alias}.${symbolName}>` : "";
                lines.push(`${indent}    export const ${symbolName}${annotation} = ${module.alias}.${symbolName};`);
                lines.push("");
            }
        }
    }
