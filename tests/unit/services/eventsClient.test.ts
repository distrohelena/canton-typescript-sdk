...
describe("EventsClient", () => {
  it("streams transactions through the selected transport", async () => {
    ...
    const transport = {
      ...
      streamTransactionsAsync: async (_request: StreamTransactionsRequest, observer: { nextAsync(event: unknown): Promise<void> }) => {
        await observer.nextAsync({ transactionId: "tx-1" });
      }
    };

    const client = new EventsClient(transport);
    await client.streamTransactionsAsync(new StreamTransactionsRequest(), { nextAsync });
