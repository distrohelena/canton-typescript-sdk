...
export function targetTemplate(templateId: TemplateId): TemplateTargetBuilder {
...
export function excludeTemplate(templateId: TemplateId): ExcludedTemplateTarget {
    return Object.freeze({
...
export function resolveDeclarativeTargets(
    ...
    for (const descriptor of descriptors) {
        ...
        if (template === undefined) {
            ...
            `Declarative target '${formatTemplateId(descriptor.templateId)}' is absent from the DAML catalog.`,
            );
        } else if (
            ...
            throw new TestingConfigurationError(
            `Declarative target '${formatTemplateId(descriptor.templateId)}:${descriptor.choice}' is absent from the DAML catalog.`,
            );
        }
    }
            ...
            if (template === undefined) {
                ...
                const choices = (descriptor.allChoices === true
                    ? template.choices.filter((choice) =>
                        !excluded.has(`${formatTemplateId(descriptor.templateId)}:${choice}`))
                    : descriptor.choices);
                ...
                if (missingChoice !== undefined) {
                    throw new TestingConfigurationError(
                    `Declarative target '${formatTemplateId(descriptor.templateId)}:${missingChoice}' is absent from the DAML catalog.`,
                    );
                }
